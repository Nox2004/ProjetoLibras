Shader "Custom/ToonShaderOutline"
{
    ...
    SubShader
    {
        ...
        Pass
		{
            ...
            float4 outline (float4 vertexPos, float outWidth)
            {
                float4x4 scale = float4x4(1 + outWidth, 0, 0, 0,
                                          0, 1 + outWidth, 0, 0,
                                          0, 0, 1 + outWidth, 0,
                                          0, 0, 0, 1 + outWidth);
			...
			{
				v2f o;
                float4 vertexPos = outline(v.vertex, _OutWidth);
				o.pos = UnityObjectToClipPos(vertexPos);
				o.uv = v.uv;
				o.normal = UnityObjectToWorldNormal(v.normal);
				o.viewDir = normalize(UnityWorldSpaceViewDir(mul(unity_ObjectToWorld, vertexPos)));
